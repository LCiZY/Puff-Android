...
import sun.bob.leela.runnable.DBImportRunnable;
...
public class SettingsActivity extends MaterialSettings {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        addItem(new TextItem(this, "import_database").setTitle(getString(R.string.import_database))
                .setSubtitle(getString(R.string.database_file_on_phone))
                .setOnclick(new TextItem.OnClickListener() {
            @Override
            public void onClick(TextItem textItem) {
                final EditText view = new EditText(getApplicationContext());
                view.setTextColor(Color.parseColor("#333333"));
                new AlertDialog.Builder(SettingsActivity.this).setTitle(getString(R.string.input_the_path_of_database_file))
                        .setView(view)
                        .setCancelable(false)
                        .setPositiveButton(R.string.yes, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface d, int which) {
                                d.dismiss();
                                dialog = ResUtil.getInstance(null).showProgressbar(SettingsActivity.this);
                                DBImportRunnable runnable = new DBImportRunnable(SettingsActivity.this, view.getText().toString().trim());
                                new Thread(runnable).run();
                            }
                        })
                        .setNegativeButton(R.string.cancel, new DialogInterface.OnClickListener() {
...
//        }
        ...
        if (event instanceof DBExportEvent) {
            final DBExportEvent dbExportEvent = (DBExportEvent) event;
            if (dbExportEvent.success) {
                new AlertDialog.Builder(this)
                        ...
                        .setNeutralButton(R.string.copy_path, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                ClipboardManager clipboard = (ClipboardManager) getApplicationContext().getSystemService(Context.CLIPBOARD_SERVICE);
                                clipboard.setText(dbExportEvent.filePath);
                                dialog.dismiss();
                            }
                        })
                        .show();
        ...
        if (event instanceof DBImportEvent){
            final DBImportEvent dbImportEvent = (DBImportEvent) event;
            if (dbImportEvent.success) {
                new AlertDialog.Builder(this)
                        .setTitle(R.string.success_em)
                        .setMessage(getString(R.string.need_relaunch))
                        .setPositiveButton(R.string.relaunch, new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                ...
                String temp = getString(R.string.please_try_again);
                if (!isNullOrEmpty(dbImportEvent.extraInfo )){
                    temp = dbImportEvent.extraInfo + getString(R.string.not_exist) + "\n" + temp;
                }
                new AlertDialog.Builder(this).setTitle(R.string.failed_em)
                        .setMessage(temp)
                        .setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
